...
<div style="margin-bottom:24px;">
    ...
    <div class="page-subtitle">View tournament standings and podium results.</div>
</div>
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px;">
        ...
        <a href="{{ route('public.standings.index') }}"
           style="padding:6px 16px;border-radius:7px;font-size:12px;font-weight:600;text-decoration:none;border:1px solid rgba(15,23,42,0.12);
                  background:{{ !request('status') ? '#22c55e' : '#fff' }};
                  color:{{ !request('status') ? '#fff' : '#0f172a' }};">All</a>
        <a href="{{ route('public.standings.index', ['status' => 'ongoing']) }}"
           style="padding:6px 16px;border-radius:7px;font-size:12px;font-weight:600;text-decoration:none;border:1px solid rgba(15,23,42,0.12);
                  background:{{ request('status')==='ongoing' ? '#22c55e' : '#fff' }};
                  color:{{ request('status')==='ongoing' ? '#fff' : '#0f172a' }};">Ongoing</a>
        <a href="{{ route('public.standings.index', ['status' => 'completed']) }}"
           style="padding:6px 16px;border-radius:7px;font-size:12px;font-weight:600;text-decoration:none;border:1px solid rgba(15,23,42,0.12);
                  background:{{ request('status')==='completed' ? '#22c55e' : '#fff' }};
                  color:{{ request('status')==='completed' ? '#fff' : '#0f172a' }};">Completed</a>
        <a href="{{ route('public.standings.index', ['status' => 'upcoming']) }}"
           style="padding:6px 16px;border-radius:7px;font-size:12px;font-weight:600;text-decoration:none;border:1px solid rgba(15,23,42,0.12);
                  background:{{ request('status')==='upcoming' ? '#22c55e' : '#fff' }};
                  color:{{ request('status')==='upcoming' ? '#fff' : '#0f172a' }};">Upcoming</a>
    </div>
    {{-- Tournament selector --}}
    <form method="GET" action="{{ route('public.standings.index') }}" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <input type="hidden" name="status" value="{{ request('status') }}">
        <select name="tournament_id"
                style="flex:1;max-width:380px;padding:9px 14px;border:1px solid rgba(15,23,42,0.12);border-radius:7px;font-size:13px;font-family:'Inter',sans-serif;color:#0f172a;background:#fff;outline:none;cursor:pointer;">
        ...
        @if(request('tournament_id'))
            <a href="{{ route('public.standings.index', ['status' => request('status')]) }}"
               style="padding:9px 16px;border-radius:7px;font-size:13px;font-weight:600;text-decoration:none;border:1px solid rgba(15,23,42,0.12);background:#fff;color:#0f172a;">Clear</a>
        @endif
    </form>
</div>
