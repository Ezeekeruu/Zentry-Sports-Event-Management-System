...
<div style="max-width:600px;margin:0 auto;">
    <div class="page-header">
        <div class="breadcrumb">ORGANIZER <span>› MATCHES › SCHEDULE</span></div>
        <div class="page-title">Schedule Match</div>
    </div>
    <div class="card">
        <form method="POST" action="{{ route('organizer.matches.store') }}">
            @csrf
            <div class="form-group">
                <label class="form-label">Tournament</label>
                <select name="tournament_id" class="form-control" required>
                    <option value="">Select your tournament</option>
                    @foreach($tournaments as $t)
                        <option value="{{ $t->id }}" {{ old('tournament_id')==$t->id?'selected':'' }}>{{ $t->tournament_name }}</option>
                    @endforeach
                </select>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div class="form-group">
                    <label class="form-label">Time <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
            ...
            <div class="form-group">
                <label class="form-label">Teams (select at least 2)</label>
