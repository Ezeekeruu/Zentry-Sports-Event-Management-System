...
<head>
    ...
    <title>Zentry — {{ $title ?? 'Player' }}</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sb-icon { width: 32px; height: 32px; background: #2e1a4a; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .sb-icon svg { width: 16px; height: 16px; fill: #a855f7; }
        ...
        .sb-item.active { color: #fff; background: rgba(168,85,247,0.12); border-left-color: #a855f7; }
        ...
        .sb-avatar { width: 28px; height: 28px; border-radius: 50%; background: #2e1a4a; font-size: 10px; font-weight: 700; color: #a855f7; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .btn-primary { background: #a855f7; color: #fff; border: none; border-radius: 7px; padding: 8px 16px; font-size: 12px; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; transition: background .12s; }
        ...
        .stat-sub { font-size: 11px; color: #a855f7; font-weight: 600; margin-top: 6px; }
        ...
        .alert-success { background: #f3e8ff; border-left: 3px solid #a855f7; color: #6b21a8; padding: 10px 14px; border-radius: 7px; font-size: 12px; font-weight: 500; margin-bottom: 16px; }
        ...
        .form-control:focus { border-color: #a855f7; box-shadow: none; }
        .form-error { font-size: 11px; color: #ef4444; margin-top: 4px; }
        .avatar { width: 34px; height: 34px; border-radius: 8px; background: #2e1a4a; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: #a855f7; flex-shrink: 0; }
    </style>
</head>
<body>
...
<div class="sidebar">
    <div class="sb-brand">
        <div class="sb-logo">
            ...
            <div>
                <div class="sb-name">ZENTRY</div>
                <div class="sb-sub">PLAYER PORTAL</div>
            </div>
        </div>
    </div>

    <div class="sb-nav">
        <div class="sb-section">My Zone</div>

        <a href="{{ route('player.dashboard') }}"
           class="sb-item {{ request()->routeIs('player.dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('player.team.show') }}"
           class="sb-item {{ request()->routeIs('player.team.*') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        ...
        <a href="{{ route('player.matches.index') }}"
           class="sb-item {{ request()->routeIs('player.matches.*') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        ...
        <a href="{{ route('player.results.index') }}"
           class="sb-item {{ request()->routeIs('player.results.*') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <path d="M9 19v-6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v6a2 2 0 0 1 2 2h14a2 2 0 0 1 2-2v-6a2 2 0 0 0-2-2h-2a2 2 0 0 0-2 2v6"/>
            </svg>
            Results
        </a>
    ...
    <div class="sb-footer">
        <div style="display:flex;align-items:center;gap:8px;">
            ...
            <div>
                ...
                <div class="sb-urole">Player</div>
            </div>
        </div>
        <form method="POST" action="{{ route('logout') }}" style="margin-top:10px;">
            @csrf
            <button type="submit" style="width:100%;background:rgba(255,255,255,0.05);border:none;color:rgba(255,255,255,0.4);font-size:11px;padding:7px;border-radius:6px;cursor:pointer;font-family:'Inter',sans-serif;">
                Sign out
            </button>
        </form>
    </div>
